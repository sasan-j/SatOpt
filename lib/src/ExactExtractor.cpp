...
bool ExactExtractor::isAvailable()
{
	...
	case 13:
	case 18:
		fileName="../ref/ExactResults/p"+std::to_string(chCount)+"_snj.txt";
		...
		infile.close();
		compareStr="Pareto "+std::to_string(swInst)+" "+std::to_string(chInst);
		//compareStr="Pareto";
		for(std::vector<std::string>::size_type i=0; i<lines.size(); i++)
		{
			...
			if(lines[i].find(compareStr)!=std::string::npos && lines[i].size()==compareStr.size())
			{
				found = true;
				i++;
				while(lines[i].find("Pareto")== std::string::npos)
				{
					...
					//std::cout << "the values read from file\n";
